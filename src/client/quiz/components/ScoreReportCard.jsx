...
export default function ScoreReportCard({
  ...
  const domainList = () => {
    ...
    questions.forEach((question) => {
      // Assuming questionRationale is the domain
      question.questionRationale.forEach((domain) => domainNumbers.add(domain));
    });
    console.log("Domain Numbers as Set:", domainNumbers); // Log domainNumbers as a Set
    const result = Array.from(domainNumbers);
    console.log("Domain List as Array:", result); // Log domainList as an Array
    return result;
  };

  // Get score per domain
  const getDomainScore = (targetDomain) => {
    if (!questions || !questions.length) return 0;
    let domainScore = 0;
    const domainQuestions = questions.filter((q) =>
      q.questionRationale.includes(targetDomain)
    );
          ...
          <div className="w-12/12 sm:w-6/12 items-center justify-between mb-4 text-center space-y-4">
            ...
            <div className="px-5">
              {domainList().map((domain) => {
                ...
                return (
                  <div key={domain}>
                    <h3 className="text-lg pt-4 pb-3 font-thin leading-none text-gray-900 dark:text-white text-left pl-1">
                      {domain}
                    </h3>
                    <div
                      ...
                      aria-valuenow={domainScore}
                      aria-valuemin="0"
                      aria-valuemax="100"
                    >
                      <div
                        className="bg-teal-700 text-sm font-semibold text-teal-100 text-center p-0.5 leading-none rounded-full"
      ...
      content: (
        <div className="tab-content pt-3">
          <h2 className="text-xl font-bold leading-none text-gray-900 dark:text-white pl-3">
            Question {question?.id || "N/A"} of {quizLength}
          </h2>
          <div className="px-5">
            <p className="my-6 font-thin text-gray-100 text-lg dark:text-gray-100">
            ...
            {selectedAnswerID === question.correctResponse ? (
              <p className="my-3 text-gray-100 text-base dark:text-teal-500">
                {" "}
                <FontAwesomeIcon
                  icon={faCheck}
                  style={{ color: "#009688" }}
              ...
              <p className="my-3 text-gray-100 text-base dark:text-red-600">
                {" "}
                <FontAwesomeIcon
                  icon={faXmark}
                  style={{ color: "#DC3545" }}
            ...
            <p className="font-bold text-gray-100 text-base dark:text-gray-300 leading-tight">
              Explanation
              <br />
              <span className="font-thin">
                {question?.questionRationale || "N/A"}
              </span>
            </p>
